...
export function initCars(screenElement: HTMLElement) {
...
export function createCar(y: number, dir: number, speed: number, index: number) {
  ...
  car.style.backgroundImage = `url("../cross-road/pictures/${carInfo.img}")`;
  if (dir === 1) {
...
export function setCarSize(car: Car, index: number) {
  const carInfo = carLib[index];
  ...
  car.element.style.backgroundImage = `url("../cross-road/pictures/${carInfo.img}")`;
