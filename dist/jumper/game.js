...
function getRandomSpawnDelay() {
    return Math.floor(Math.random() * (platformSpawnMax - platformSpawnMin + 1)) + platformSpawnMin;
}
...
function isColliding(jumper, platform) {
    ...
    const horizontalHit = jRight > pLeft + 2 &&
        jLeft < pRight - 2;
    const crossedFromAbove = prevBottom <= pTop &&
        jBottom >= pTop;
