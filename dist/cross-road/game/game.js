...
import { initCars, createCar, moveCarsX, redrawCars, removeCarsOutsideScreen, getRandomCarDirection, getRandomCarSpeed, getRandomCarDistance, setCarSize, getRandomCarIndex, } from "./cars.js";
import { initRivers, createRiver, moveLogsX, redrawRivers, removeRiversOutsideScreen, getRandomRiverDirection, getRandomRiverSpeed, } from "./rivers.js";
...
initCars(screen);
initRivers(screen);
createCar(state.chickenY - 4 * ChickenHeight, getRandomCarDirection(), getRandomCarSpeed(), getRandomCarIndex());
createRiver(state.chickenY - 8 * ChickenHeight, getRandomRiverDirection(), getRandomRiverSpeed());
...
setInterval(function () {
    if (state.gameOver)
        return;
    if (state.cars.length + state.rivers.length >= 30)
        return;
    ...
    if (isRiver) {
        createRiver(y, getRandomRiverDirection(), getRandomRiverSpeed());
    }
    else {
        createCar(y, getRandomCarDirection(), getRandomCarSpeed(), getRandomCarIndex());
    }
    state.lastLaneY = y;
}, 100);
setInterval(function () {
